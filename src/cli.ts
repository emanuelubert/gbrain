...
async function handleCliOnly(command: string, args: string[]) {
  ...
  try {
    switch (command) {
      ...
      case 'facts': {
        // S195 Phase 1 — enrich_stubs §I8 operator-correction persistence.
        const { runFacts } = await import('./commands/facts.ts');
        await runFacts(engine, args);
        break;
      }
      case 'enrich-stubs': {
        // S196 Phase 5 — backward-mode + on-demand enrich_stubs CLI.
        // Forward-mode runs via dream-cycle; this is the operator-triggered
        // path for backward bulk + debugging.
        const { runEnrichStubs } = await import('./commands/enrich-stubs.ts');
        await runEnrichStubs(engine, args);
        break;
      }
      case 'founder': {
